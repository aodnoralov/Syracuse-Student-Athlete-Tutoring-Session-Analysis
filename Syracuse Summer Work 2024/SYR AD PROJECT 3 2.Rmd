...
Grades <- Grades[!(GradesTidy$SUID == 280657728 & Grades$`Acad Prog Sh Desc` == "BrDgtJrnl"), ]
...
#Sessions$`Canceled By`[is.na(Sessions$`Canceled By`)] <- "Not Canceled"
...
#Sessions$`Location`[is.na(Sessions$`Location`)] <- "Not Specified"
...
SessionNotes <- rbind(SessionNotesInstructionalAssistants,SessionNotesTutors)
...
#SessionNotes$`Actual Start Time`[is.na(SessionNotes$`Actual Start Time`)] <- "Has Not Occurred"
#SessionNotes$`Actual End Time`[is.na(SessionNotes$`Actual End Time`)] <- "Has Not Occurred"
#SessionNotes$`Actual Duration`[is.na(SessionNotes$`Actual Duration`)] <- "Has Not Occurred"
...
#SessionNotes$`Excused`[is.na(SessionNotes$`Excused`)] <- "Yes"
...
#SessionNotes$`Canceled By`[is.na(SessionNotes$`Report Filed At`)] <- "Not Cancelled"

#SessionNotes$`Notes`[is.na(SessionNotes$`Notes`)] <- "No Notes"
#SessionNotes$`What tasks or materials were covered during this session?`[is.na(SessionNotes$`What tasks or materials were covered during this session?`)] <- "No Answer"
...
#SessionNotes$`What does this student need to do prior to their next session?`[is.na(SessionNotes$`What does this student need to do prior to their next session?`)] <- "No Answer"
...
#SessionNotes$`Excused` <- ifelse(SessionNotes$`Excused`=="No",0,1)
...
write_csv(Tutors, "SYR AD TUTORIAL PROJECT/TutorsTidy.csv")
```

```{r}
Fall23 <- read_csv("Course Enrollment - Fall 2023(in).csv")
Spring24 <- read_csv("Course Enrollment - Spring 2024(in).csv")
